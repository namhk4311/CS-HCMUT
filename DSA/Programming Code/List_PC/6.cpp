...
void LList<E>::reverse() {
    ...
    Link<E> reverseLL = new Link<E>;
    if (curr_pos == 0) curr = reverseLL;
    for (int j = 1; j <= cnt; j++) {
        Link<E> *tmp2 = new Link<E>(llvalue.top());
        llvalue.pop();
        if (curr_pos == j) curr = tmp2;
        if (j == cnt) tail = tmp2;
        Link<E> *tmp3 = reverseLL;
