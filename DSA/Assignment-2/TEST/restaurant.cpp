...
void print(priority_queue<Huffnod *, vector<Huffnod *>, My_compare> q) {
	//solution << q.size() << endl;
	while (q.size()) {
		solution << q.top()->root_char() << " " << q.top()->root_val() << endl;
		if (q.top()->letter == '~') solution << q.top()->val << ":" << q.top()->l->letter << " " << q.top()->r->letter << endl;
		q.pop();
	}
	solution << "============================" << endl;
} 

void print2D(Huffnod *root, int space) {
    if (root == nullptr) return;
    space += SPACE;
    print2D(root->r, space);
    solution << endl;
    for (int i = SPACE; i < space; i++) {
        solution << " ";
    } 
    solution << ((root->letter == '~')? "[" :  "(") << root->val << ((root->letter == '~')? "" : "," ); 
    if (root->letter != '~') solution << root->letter;
    solution << ((root->letter == '~')? "]" : ")") << "\n";
    print2D(root->l, space);
}
...
class BSTtree {
	...
	public:
		...
		// BSTNode *searchNode(BSTNode *root, BSTNode *findnode) { deletenow
		// 	if (!root) return nullptr;
		// 	if (root == findnode) {
		// 		return findnode;
		// 	}
		// 	else if (findnode->r->Result < root->r->Result) {
		// 		return searchNode(root->pLeft, findnode);
		// 	}
		// 	else if (findnode->r->Result >= root->r->Result) {
		// 		return searchNode(root->pRight, findnode);
		// 	}
		// 	return nullptr;
		// }
		...
		void deleteFIFO(ull Y) {
			...
			Y = Y % MAXSIZE;
			//===============================
			number_KOKUSEN.push_back((int)Y);
		...
		void print2D(BSTNode *root, int space) {
			...
			solution << root->r->Result << "\n";
			print2D(root->pLeft, space);
		}
...
class GojoRestaurant {
	...
	public:
		...
		GojoRestaurant(int MAXSIZE) {
			capacity = MAXSIZE;
			sizeGojoArea = 0;
			Gojoarea = new BSTtree*[capacity]; ++COUNT;
			update = new status[capacity]; ++COUNT;
			postorderlist = new int*[capacity]; ++COUNT;
		...
		//===================================================
		void printResult(BSTNode *root, string& s) {
			if (root) {
				s += to_string(root->r->Result); 
				if (!root->pLeft && !root->pRight) return;
				s += "(";
				printResult(root->pLeft,s);
				s += ",";
				printResult(root->pRight,s);
				s += ")";
			}
			else {
				s+= "NULL";
		...
		void KOKUSENimplement() {
			for (int i = 0; i < capacity; i++) {
				//print result
				string result_root_print_before = "";
				string result_root_print_after = "";
				//=================================
				if (update[i] == NON_EMPTY && Gojoarea[i]->numOfGojoCus()) {
					//============================
					result_root_print_before += "ID="+ to_string(i + 1) + " : ";
					result_root_print_after += "ID=" + to_string(i + 1) + " : ";
				
					printResult(Gojoarea[i]->Root(), result_root_print_before);
					root_before_KOKUSEN.push_back(result_root_print_before);
					//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
					postorderlist[i] = new int[Gojoarea[i]->numOfGojoCus()]; ++COUNT;// coi lai postorderlist co can dung xuyen suot btl ko, neu ko thi chi can cap phat o trong ham thoi dc roi
					int idx = 0;
					...
					//======================================
					if (Gojoarea[i]->numOfGojoCus()) {
					printResult(Gojoarea[i]->Root(), result_root_print_after);
					root_after_KOKUSEN.push_back(result_root_print_after); 
					}
		...
		// void print() { deletenow
		// 	for (int i = 0; i < capacity; i++) {
		// 		if (update[i] == NON_EMPTY) {
		// 			solution << "Area " << i + 1 << ": num of cus = " << Gojoarea[i]->numOfGojoCus() << endl;
		// 			Gojoarea[i]->printNow();
		// 		}
		// 	}
		// }
		...
		~GojoRestaurant() {
			...
			delete[] postorderlist; --COUNT;
			delete[] update; --COUNT;
			delete[] Gojoarea; --COUNT;
...
class SukunaRestaurant {
	...
	public:
		...
		SukunaRestaurant(int MAXSIZE){
			...
			heap = new SuArea*[this->capacity]; ++COUNT;
			update = new status[this->capacity]; ++COUNT;
			sizeHeap = 0;
		...
		// void DeleteHeap() { deletenow
		// 	if (sizeHeap == 0) return;
		// 	int ID = heap[0]->label;
		// 	if (sizeHeap > 1) {
		// 		swap(heap[sizeHeap - 1]->updatePosInHeap, heap[0]->updatePosInHeap);
		// 		SuArea *tmp = heap[0];
		...
		// 		--sizeHeap;
		// 	}
		// 	//labelOrder->remove(ID);
			
		// }

		
		// void printdeque(deque<Customer*>d) { deletenow
		// 	while (d.size()) {
		// 		//solution << "(" << d.front()->Result << ")" << ",";
		// 		solution << "(" << d.front()->Result << ")" << ",";
		// 		d.pop_front();
		// 	}
		// }
		...
		// void print() { deletenow
		// 	for (int i = 0; i < sizeHeap; i++) {
		// 		solution << "Label: " << heap[i]->label << " " << "NumOfCus: " << heap[i]->NUM << " " << "Appearance: " << heap[i]->appearance_order << " ";
		// 		solution << "List: ";
		// 		printdeque(heap[i]->area);
		// 		solution << " " << i << endl;
				
		// 	}
		// 	solution << sizeHeap << endl;
		// }		
		...
		// void printListOrder(priority_queue<SuArea*, vector<SuArea*>, AppearanceCompare> pq) { deletenow
		// 	solution << "List to delete:" << endl;
		// 	while (!pq.empty()) {
		// 		solution << "Label: " << pq.top()->label << " " << "Appearance: " << pq.top()->appearance_order << endl;
		// 		pq.pop();
		// 	}
		// 	solution << "===================================\n";
		// }
		...
		//===============================================
		void printArea(deque<Customer *> s, string &str, int label) {
			str += "ID=" +  to_string(label) + " : ";
			while (s.size()) {
				str += to_string(s.back()->Result) + " " ;
				s.pop_back();
				// str += ((s.size() == 0) ? "" : " ");
			}
		}
		...
		void KEITEIKENimplement(int NUM) {
			...
			//===============================================
			for (int i = 0; i < sizeHeap; i++) {
				Heap_before_KEITEIKEN.push_back(make_pair(heap[i]->label, heap[i]->NUM));
				string s = "";
				printArea(heap[i]->area, s, heap[i]->label);
				customers_before_KEITEIKEN.push_back(s);
			}
			...
			while (!printRes.empty()) {
				solution << printRes.front().first << "-" << printRes.front().second << endl;
				printRes.pop();
			}

			//=================================================
			for (int i = 0; i < sizeHeap; i++) {
				Heap_after_KEITEIKEN.push_back(make_pair(heap[i]->label, heap[i]->NUM));
				string s = "";
				printArea(heap[i]->area, s, heap[i]->label);
				customers_after_KEITEIKEN.push_back(s);
			}
			//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
		}
...
class Implement{
	...
	// Customer *recentCus = nullptr;
	// Hufftree *recentTree = nullptr;
...
public:
	...
	void init() {
		gojo = new GojoRestaurant(MAXSIZE); ++COUNT;
		sukuna = new SukunaRestaurant(MAXSIZE); ++COUNT;
	}
	Implement() {
		init();
	}
	...
	void traverseTree(Huffnod *root, string &s) {
		if (root) {
			if (root->letter == '~') {
			...
			s += "(";
			traverseTree(root->l, s);
			s += ",";
			traverseTree(root->r, s);
			s += ")";
		}
		else {
			s += "NULL";
	...
	void LAPSE(string name)
	{
		if (name.length() < 3) return;
			...
			for (unordered_map<char, int>::iterator it = frequencyChar.begin(); it != frequencyChar.end(); ++it) {
				//solution << it->first << " " << it->second << endl;
			...
			//print nguoc=======================================
			for (int i = listFreqEncrypted.size() - 1; i >= 0; i--) {
				freq_LAPSE.push_back(make_pair(listFreqEncrypted[i].character, listFreqEncrypted[i].frequency));
			}
			...
			if (!success) {
				...
				getValue(tree->R(), "", vtr);
				traverseTree(tree->R(), root_LAPSE);
				printHuff(tree->R(), recentCustomer);
			}
	...
	void HAND(){
		solution << recentCustomer;
	}
	void LAPSE2(string name) {
		int result = stoi(name);
		//solution << "result = " <<  name << endl;
		Hufftree *tree = new Hufftree(); ++COUNT;
		Customer *newCus = new Customer("", stoi(name), tree);
		//recentCus = newCus;
		++COUNT;
		if (stoi(name) % 2) {
			//solution << "Gojo team\n";
			...
			//if (stoi(name) == 168) solution << "hihiii" << endl;
			//solution << "Sukuna team\n";
			sukuna->InsertCustomer(newCus);
		}
	}
...
private:
	Implement *r;
public:
	JJK_RESTAURANT_OPERATIONS() {r = new Implement();}
	void LAPSE(string name)
	{
	
		if(name[0] >= '0' && name[0] <= '9') //! Bỏ đi
		{
			//TODO
			r->LAPSE2(name);
			return;
		...
		r->LAPSE(name);
		//TODO cập nhật freq_LAPSE  và result_LAPSE root_LAPSE theo đúng mô tả
		// freq_LAPSE = {};
		// result_LAPSE = 0;
		// root_LAPSE = "";
	}	
	...
	//* xử lí nhà hàng gojo
	void KOKUSEN(){
		//TODO cập nhật number_KOKUSEN  root_before_KOKUSEN  root_after_KOKUSEN
		r->KOKUSEN();
		// number_KOKUSEN = {};
		// root_before_KOKUSEN = {};
		// root_after_KOKUSEN = {};
	...
	void LIMITLESS(int num){
		//TODO print giống thầy mô tả dùng solution
		r->LIMITLESS(num);
	}
	...
	~JJK_RESTAURANT_OPERATIONS() {delete r;}
};
